...
local function addCompilerProps(cfg)
	...
	writeString('NvccCompilation', cfg.cudaNvccCompilation)
	writeString('CudaRuntime', cfg.cudaRuntime)
	writeString('CompileOut', cfg.cudaCompileOut)
...
local function addLinkerProps(cfg)
    if cfg.cudaLinkerOptions ~= nil then
		...
		writeString('PerformDeviceLink', cfg.cudaLinkerOptions)
        premake.w('</CudaLink>')
    end
end
